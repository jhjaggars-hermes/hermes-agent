...
class DiscordAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            async def on_message(message: DiscordMessage):
                ...
                _role_mentioned = self._has_accepted_role_mention(message)
                if not isinstance(message.channel, discord.DMChannel) and (
                    message.mentions or _raw_self_mention or _role_mentioned
                ):
                    _self_mentioned = _raw_self_mention or _role_mentioned
    ...
    def _discord_accepted_role_ids(self) -> set[str]:
        """Return Discord role IDs that count as explicit bot invocations.
